...
def run_gui(ini_path: Path) -> None:
    ...
    def run_release():
        ...
        def worker():
            ...
            # Update rolling 'portable' tag release
            log("Updating 'portable' release tag...\n")
            try:
                # Delete existing 'portable' release and tag if present, then recreate
                del_proc = subprocess.run(
                    ["gh", "release", "delete", "portable", "--yes", "--cleanup-tag"],
                    capture_output=True, text=True, cwd=str(project_root)
                )
                if del_proc.returncode == 0:
                    log("  Removed previous 'portable' release.\n")
                else:
                    log("  No existing 'portable' release to remove (or already gone).\n")

                portable_cmd = [
                    "gh", "release", "create", "portable", str(archive_path),
                    "--title", f"portable ({version})",
                    "--notes", f"Latest portable build — {version}",
                    "--latest=false"
                ]
                if not run_cmd_sequence([portable_cmd], cwd=project_root):
                    log("'portable' release upload failed.\n")
                else:
                    log("'portable' release updated successfully.\n")
            except FileNotFoundError:
                log("GH CLI not found. Cannot update 'portable' release.\n")
            except Exception as e:
                log(f"'portable' release failed: {e}\n")
            finally:
                log("\n>>> Release complete <<<\n")
                set_ui_busy(False)
                proc_state['proc'] = None
