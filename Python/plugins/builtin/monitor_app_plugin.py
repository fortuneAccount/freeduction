...
class MultiMonitorToolPlugin(ToolPlugin):
    """Plugin for NirSoft Multi Monitor Tool"""
    ...
    def get_config_schema(self) -> Dict[str, ConfigField]:
        return {
            'monitor_game_cfg': ConfigField(
                name='monitor_game_cfg',
                field_type='file',
                label='Game Monitor Config',
                required=False,
                filter='Config Files (*.cfg);;All Files (*.*)',
                help_text='Monitor configuration for gaming'
            ),
            'monitor_desk_cfg': ConfigField(
                name='monitor_desk_cfg',
                field_type='file',
    ...
    def build_launch_command(self, config: PluginConfig) -> Optional[str]:
        ...
        monitor_game_path = config.get_field('monitor_game_cfg')
        if not monitor_game_path or not os.path.exists(monitor_game_path):
            return None
